...
export async function exportHealthData(
  ...
  try {
    ...
    const [profile, vitals, meds, notifs] = await Promise.all([
      ...
      patientId ? vitalsService.getVitalsHistory(patientId) : ([] as VitalsRecord[]),
      patientId ? medicationService.getMedications(patientId) : ([] as Medication[]),
      notificationService.getNotifications(userId),
    ]);

    console.log(TAG, `Fetched → profile: ${profile ? 'yes' : 'no'}, vitals: ${vitals.length}, meds: ${meds.length}, notifs: ${notifs.length}`);
    ...
    const summaryData: (string | number)[][] = [
      ...
      [isAr ? 'إجمالي قراءات العلامات الحيوية' : 'Total Vitals Records', vitals.length],
      ...
      [isAr ? 'الإشعارات غير المقروءة' : 'Unread Notifications', notifs.filter((n) => !n.is_read).length],
    ];

    // Add vitals stats if data exists
    if (vitals.length > 0) {
      const hrs = vitals.map((v) => v.heart_rate).filter((v): v is number => v != null);
      const spo2s = vitals.map((v) => v.oxygen_saturation).filter((v): v is number => v != null);
      const sysBPs = vitals.map((v) => v.blood_pressure_systolic).filter((v): v is number => v != null);

      summaryData.push(['', ''], [isAr ? '── ملخص العلامات الحيوية ──' : '── Vitals Summary ──', '']);

      if (hrs.length > 0) {
        const avgHR = Math.round(hrs.reduce((a, b) => a + b, 0) / hrs.length);
        summaryData.push(
          [isAr ? 'متوسط معدل القلب' : 'Avg Heart Rate', `${avgHR} bpm`],
          [isAr ? 'أدنى معدل قلب' : 'Min Heart Rate', `${Math.min(...hrs)} bpm`],
          [isAr ? 'أعلى معدل قلب' : 'Max Heart Rate', `${Math.max(...hrs)} bpm`],
        );
      }
      if (spo2s.length > 0) {
        const avgO2 = Math.round(spo2s.reduce((a, b) => a + b, 0) / spo2s.length);
        summaryData.push(
      ...
      if (sysBPs.length > 0) {
        const avgBP = Math.round(sysBPs.reduce((a, b) => a + b, 0) / sysBPs.length);
        summaryData.push(
          [isAr ? 'متوسط ضغط الدم الانقباضي' : 'Avg Systolic BP', `${avgBP} mmHg`],
        );
      }
    }
    ...
    XLSX.utils.book_append_sheet(wb, vitalsWs, isAr ? 'العلامات الحيوية' : 'Vitals');
    ...
    if (vitals.length > 0) {
      ...
      const chartWs = XLSX.utils.aoa_to_sheet([instructionRow, [''], chartHeaders, ...chartRows]);
      setColWidths(chartWs, [20, 14, 10, 14, 14]);
      chartWs['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 4 } }];
      XLSX.utils.book_append_sheet(wb, chartWs, isAr ? 'بيانات الرسوم' : 'Charts Data');
    }
    ...
    XLSX.utils.book_append_sheet(wb, notifWs, isAr ? 'الإشعارات' : 'Notifications');
